function [chk,sol]=MomentRelaxation(Qse,Qsw,Qsobj,QQ,usecvx)
n           =   size(Qse{1},1)-1;

tic;
[is,js,spM] =   GetInds(Qse,Qsw,Qsobj);
...
if(~usecvx)
    ...
    for it=1:length(Qsobj)
        prog    =   prog.withEqs(trace(y(Yi(1:n+1,1:n+1))'*Qsobj{it}));
        ix      =   FindClique(Qsobj{it},spM);
        prog    =   prog.withEqs(GetY(ix,Qsobj{it}));
    end
    ...
    GetY        =   @(anyix,anyM) GetYconst(anyM,y,kf,anyix);
    for it=1:length(Qsobj)
        trace(y(Yi(1:n+1,1:n+1))'*Qsobj{it})==0;
        ix=FindClique(Qsobj{it},spM);
        GetY(ix,Qsobj{it})==0;
    end
    trace(y(Yi(1:n+1,1:n+1))'*QQ)==0;
...
function [is,js,spM]=GetInds(Qse,Qsw,Qsobj)
n           =   size(Qse{1},1);
M           =   zeros(n);
for it=1:length(Qse)
    M   =   M+CF(Qse{it});
